...
public:
    ...
    void init(const ILP_input &input, const bdd_order o = bdd_order::given_order);
    void init(const bdd_order o = bdd_order::given_order);
...
protected:
    ...
    std::vector<BDD_BRANCH_INSTRUCTION> bdd_branch_nodes_;
    std::vector<std::size_t> bdd_delimiters; // TODO: still needed?

    struct Lagrange_multiplier
    {
        std::size_t bdd_nr;
        std::size_t bdd_branch_node_offset;
        BDD_VARIABLE bdd_variable;
    };
    ...
    struct bdd_variable_delimiter
    {
        std::size_t variable;
        std::size_t bdd_branch_node_offset;
        std::size_t Lagrange_multipliers_index;
    };
...
template<typename BDD_BRANCH_INSTRUCTION, typename BDD_VARIABLE>
void bdd_base_consecutive<BDD_BRANCH_INSTRUCTION, BDD_VARIABLE>::init(const bdd_order order)
{
    ...
    const auto bdd_order = [&]() {
        if (order == bdd_order::given_order)
            return given_bdd_order();
        if (order == bdd_order::Cuthill_McKee)
            return find_cuthill_mkkee_order();
        if (order == bdd_order::minimum_degree)
            return find_minimum_degree_bdd_ordering();
        if (order == bdd_order::BFS)
    ...
    struct Lagrange_multiplier_tmp
    {
        std::size_t bdd_nr;
        std::size_t bdd_branch_node_offset; };
        ...
        for(std::size_t i=0; i<bdd_order.size(); ++i) {
            ...
            std::unordered_map<std::size_t, BDD_BRANCH_INSTRUCTION*> bdd_storage_index_to_branch_node_index;

            auto bdd_branch_node_index_from_bdd_storage_index = [&](const std::size_t idx) -> BDD_BRANCH_INSTRUCTION* {
                if(idx == bdd_storage::bdd_node::terminal_0)
                    return BDD_BRANCH_INSTRUCTION::terminal_0();
                else if(idx == bdd_storage::bdd_node::terminal_1)
                    return BDD_BRANCH_INSTRUCTION::terminal_1();
            ...
            for(std::size_t bdd_storage_node_idx = first_bdd_node; bdd_storage_node_idx<last_bdd_node; ++bdd_storage_node_idx) {
                ...
                if(!BDD_BRANCH_INSTRUCTION::is_terminal(bdd_low_outgoing)) {
                ...
                if(!BDD_BRANCH_INSTRUCTION::is_terminal(bdd_high_outgoing)) {
        ...
        */
        ...
        for(std::size_t i=0; i<nr_bdds(); ++i) {
            for(std::size_t j=0; j<nr_variables(i); ++j) {
                ...
                double* Lagrange_mult = &Lagrange_multipliers(variable, current_bdd_var_delimiter.Lagrange_multipliers_index).bdd_variable.cost;
                for(std::size_t bdd_node_idx = first_bdd_node; bdd_node_idx<last_bdd_node; ++bdd_node_idx) {
...
template <typename BDD_BRANCH_INSTRUCTION, typename BDD_VARIABLE>
template <typename ITERATOR>
void bdd_base_consecutive<BDD_BRANCH_INSTRUCTION, BDD_VARIABLE>::set_costs(ITERATOR begin, ITERATOR end)
{
    ...
    for (std::size_t v = 0; v < std::distance(begin, end); ++v)
    {
        ...
        {
            Lagrange_multipliers(v, j).bdd_variable.cost = cost;
        }
    }
    for (std::size_t v = std::distance(begin, end); v < Lagrange_multipliers.size(); ++v)
    {
        for (std::size_t j = 0; j < Lagrange_multipliers[v].size(); ++j)
        {
            Lagrange_multipliers(v, j).bdd_variable.cost = 0.0;
        }
    }
}

template<typename BDD_BRANCH_INSTRUCTION, typename BDD_VARIABLE>
std::array<std::size_t, 2> bdd_base_consecutive<BDD_BRANCH_INSTRUCTION, BDD_VARIABLE>::bdd_branch_node_range(const std::size_t bdd_nr) const
{
...
template <typename BDD_BRANCH_INSTRUCTION, typename BDD_VARIABLE>
std::array<std::size_t, 2> bdd_base_consecutive<BDD_BRANCH_INSTRUCTION, BDD_VARIABLE>::bdd_branch_node_range(const std::size_t bdd_nr, const std::size_t variable_index) const
{
    assert(bdd_nr < nr_bdds());
...
template <typename BDD_BRANCH_INSTRUCTION, typename BDD_VARIABLE>
std::size_t bdd_base_consecutive<BDD_BRANCH_INSTRUCTION, BDD_VARIABLE>::bdd_branch_node_index(const bdd_branch_node_opt *bdd) const
{
...
template <typename BDD_BRANCH_INSTRUCTION, typename BDD_VARIABLE>
std::vector<std::size_t> bdd_base_consecutive<BDD_BRANCH_INSTRUCTION, BDD_VARIABLE>::bdd_branch_node_variables() const
{
...
template <typename BDD_BRANCH_INSTRUCTION, typename BDD_VARIABLE>
two_dim_variable_array<std::size_t> bdd_base_consecutive<BDD_BRANCH_INSTRUCTION, BDD_VARIABLE>::bdd_adjacency() const
{
...
template <typename BDD_BRANCH_INSTRUCTION, typename BDD_VARIABLE>
permutation bdd_base_consecutive<BDD_BRANCH_INSTRUCTION, BDD_VARIABLE>::given_bdd_order() const
{
    permutation order;
    for (std::size_t i = 0; i < bdd_storage_.nr_bdds(); ++i)
        order.push_back(i);
...
template <typename BDD_BRANCH_INSTRUCTION, typename BDD_VARIABLE>
permutation bdd_base_consecutive<BDD_BRANCH_INSTRUCTION, BDD_VARIABLE>::find_cuthill_mkkee_order() const
{
...
template <typename BDD_BRANCH_INSTRUCTION, typename BDD_VARIABLE>
permutation bdd_base_consecutive<BDD_BRANCH_INSTRUCTION, BDD_VARIABLE>::find_minimum_degree_bdd_ordering() const
{
    const two_dim_variable_array<std::size_t> bdd_adj = bdd_adjacency();
...
template <typename BDD_BRANCH_INSTRUCTION, typename BDD_VARIABLE>
permutation bdd_base_consecutive<BDD_BRANCH_INSTRUCTION, BDD_VARIABLE>::find_bfs_bdd_ordering() const
{
    const two_dim_variable_array<std::size_t> bdd_adj = bdd_adjacency();
